...
export default function CommentsSection({ exampleSlug }: CommentsSectionProps) {
  const { user } = useAuth();
  ...
  const loadComments = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/examples/${exampleSlug}/comments`);
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setUpgradeMessage("Please sign in to comment");
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          setUpgradeMessage(data.message || "Please sign in to comment");
          setRequiresUpgrade(false);
          setShowUpgradeModal(true);
